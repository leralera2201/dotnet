...
namespace Lab1.Controllers
{
    ...
    public class TagController : ControllerBase
    {
        ...
        [Route("")]
        [HttpGet]
        public async Task<IEnumerable<Tag>> GetAll([FromQuery] TagParameters parameters)
        {
        ...
        [Route("")]
        [HttpPost]
        public async Task<ActionResult> Create([FromBody] TagRequestDto dto)
        {
        ...
        [Route("{id}")]
        [HttpPut]
        public async Task<ActionResult> Update(int id, [FromBody] TagRequestDto dto)
        {
            var validator = new TagValidator();
            var result = await validator.ValidateAsync(dto);
            if (!result.IsValid) return BadRequest(result.Errors);
